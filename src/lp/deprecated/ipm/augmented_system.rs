...
use crate::{
    ...
    lp::LinearProgram,
    state::{Delta, Residuals, SolverState},
};
...
pub trait AugmentedSystem<'a, Solver: LinearSolver> {
    ...
    fn solve(&mut self, view: &'a IPMView, rhs: &RHS) -> Result<Delta, Problem>;

    /// Solves for a search direction reusing the current factorization.
    fn resolve(&mut self, state: &'a IPMView, rhs: &RHS) -> Result<Delta, Problem>;
}
...
impl<'a, Solver: LinearSolver> AugmentedSystem<'a, Solver> for SlackReducedSystem<'a, Solver> {
    ...
    fn solve(&mut self, state: &SolverState, rhs: &RHS) -> Result<SearchDirection, Problem> {
        ...
        let xl_inv = cwise_inverse((&vars.x - &self.lp.l).as_ref());
        let xu_inv = cwise_inverse((&vars.x - &self.lp.u).as_ref());
        let sys_diag = cwise_multiply(xl_inv.as_ref(), vars.z_l.as_ref())
            + cwise_multiply(xu_inv.as_ref(), vars.z_u.as_ref());
    ...
    fn resolve(&mut self, state: &SolverState, rhs: &RHS) -> Result<SearchDirection, Problem> {
        ...
        let (r_d, r_c, r_l, r_u) = (rhs.dual(), rhs.primal(), rhs.slack_l(), rhs.slack_u());
